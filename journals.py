...
class GummyAbstJournal(metaclass=ABCMeta):
    ...
    TITLE_TAG = "title"  # TODO: h1タグではなくtitleタグの方が適切では?
    SECTION_TAG = "section"  # sectionタグだったが、markdown-pdfで.md->htmlにexportした際にsectionタグは生成されない為h2に変更
    HEADER_TAG = "h2"
    SUBHEADER_TAG = "h3"
    PARAGRAPH_TAG = "p"
    IMAGE_TAG = "img"
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.subheader_tag_names = subheadTags
        self.crawling_logs = {}
    ...
    def get_contents(
        self, url: str, driver: Optional[webdriver.Chrome] = None, crawl_type: Optional[str] = None, **gatewaykwargs
        ...
        handleKeyError(lst=SUPPORTED_CRAWL_TYPES, crawl_type=crawl_type)  # 指定したcrawl_typeがサポートしているものか否か

        if self.verbose:
            print(f"Crawling Type: {toACCENT(crawl_type)}")

        # ここでcrawl_typeに応じて、複数のcontents_funcを使い分けているっぽい
        # build-in関数getattr():メソッド名を入力せずに、文字列の内容を元にメソッドを呼び出せる
        # getattr(a, "hogehoge_method")は、a.hogehoge_methodと等価
        # get_contents_func:self.get_contents_soup = getattr(self, name=f"get_contents_{crawl_type}")

        if crawl_type == "soup":
            print(f"[DEBUG]crawl_type:{crawl_type}. so conduct get_contents_soup")
            return self.get_contents_soup(url=url, driver=driver, **gatewaykwargs)
        if crawl_type == "pdf":
            return self.get_contents_pdf(url=url, driver=driver, **gatewaykwargs)
        if crawl_type == "tex":
            return self.get_contents_tex(url=url, driver=driver, **gatewaykwargs)

    # ================== #
    #  crawl_type="soup" #
    # ================== #

    # TODO: 不要なstaticメソッドな気がする
    @staticmethod
    ...
    def get_contents_soup(
        self, url: str, driver: Optional[webdriver.Chrome] = None, **gatewaykwargs
    ) -> Tuple[str, List[ContentInSection]]:
        ...
        """
        # htmlのソースコードを取得する
        target_url = self.get_soup_url(url)
        print(f"[LOG]target_url:{target_url}")
        soup = self.get_soup_source(url=target_url, driver=driver, **gatewaykwargs)

        # ソースコードからtitleを取得
        title = self.get_title_from_soup(soup)
        print(f"[LOG] title : {title}")

        # htmlソースコードを各セクション(sectionタグ)毎に分ける
        soup_sections = self.get_sections_from_soup(soup)
        print(f"[LOG] length of soup_sections : {len(soup_sections)}")

        # 各セクションのソースからtagとtextをdictで抽出する(Dict["tag名("raw", "head", "subhead", "img"のどれか一つ)":テキスト])
        contents = self.get_contents_from_soup_sections(soup, soup_sections)
        print(f"[LOG] length of contents_in_all_sections : {len(contents)}")

        # self._store_crawling_logs(soup=soup, title=title, soup_sections=soup_sections, contents=contents)

        return title, contents

    def get_soup_source(self, url: str, driver: Optional[webdriver.Chrome] = None, **gatewaykwargs) -> BeautifulSoup:
        ...
        html_extracted: str = driver.page_source  # TODO: encode("utf-8")でbytes型に変換する必要なくないか...??
        print(f"[debug] type(html_extracted) {type(html_extracted)}")
        # print(f"[debug]html: {html_extracted}")

        # debug用のログとして、htmlを保存
        with open("html_extracted_for_begug.html", mode="w", encoding="utf-8") as f:
            f.write(html_extracted)

        soup = BeautifulSoup(html_extracted, "html.parser")
        soup = self.decompose_unnecessary_tags(soup=soup)

        # debug用のログとして、htmlを保存
        with open("html_decomposed_for_begug.html", mode="w", encoding="utf-8") as f:
            f.write(soup.__str__())
        return soup

    def make_elements_visible(self, driver: webdriver.Chrome):
    ...
    def decompose_unnecessary_tags(self, soup: BeautifulSoup) -> BeautifulSoup:
    ...
    def get_title_from_soup(self, soup: BeautifulSoup) -> str:
        """Get page title from page source.(最初に出てきたh1タグをtitleとみなす)
        ...
        Returns:
            str : A page title.
        """
        title = find_target_text(soup=soup, target_tag=GummyAbstJournal.TITLE_TAG, default_value=self.default_title)
        return title

    def get_sections_from_soup(self, soup: BeautifulSoup) -> List[bs4.element.Tag]:
        ...
        """
        sections: List[bs4.element.Tag] = soup.find_all(
            GummyAbstJournal.SECTION_TAG,
            recursive=True,
        )  # aTODO: h2のsection_tagにh3のセクションタグが含まれてしまっている。また更にh3のセクションタグも取得してしまっている。

        print(f"[LOG] len(sections):{len(sections)}")

        if len(sections) == 0:
            print(f"[WARNING] section tag is not found_all.")

        return sections

    def get_contents_from_soup_sections(
        self, soup_decomposed: bs4.BeautifulSoup, soup_sections: List[bs4.element.Tag]
    ) -> List[ContentInSection]:
        ...
        # 各セクションタグのhtmlに対して繰り返し処理
        for i, section in enumerate(soup_sections):
            section_head_tag = section.find(name=GummyAbstJournal.HEADER_TAG)
            if section_head_tag is None:
                section_head_tag = section.find(name=GummyAbstJournal.SUBHEADER_TAG)
            if section_head_tag is None:
                section_head_tag = section.find(name="h4")
            if section_head_tag is None:
                print(f"[LOG]header_str: {i}/{len_soup_sections}:this section don't have h2, h3, h4 tag.")
                continue
            section_head_str = str_strip(section_head_tag.get_text())
            print(f"[LOG]header_str: {i}/{len_soup_sections}:{section_head_str}")

            # TODO: ここでcontentsを取り出すタイミングで重複が発生している。
            # contents_in_one_section = self.organize_soup_section(section_tag=section)
            print(f"==================")

        return self.organize_soup_section(soup_decomposed)

    def organize_soup_section(self, section_tag: bs4.element.Tag) -> List[ContentInSection]:
        ...
        """
        contents_in_one_section = []

        # TODO: ここで失敗になっている
        # splitted_soup: List[bs4.element.Tag] = split_section(
        #     section_tag=section_tag,
        #     split_tag_names=self.subheader_tag_names + ["img"],
        # )

        splitted_soup: List[bs4.element.Tag] = section_tag.find_all(
            name=GummyAbstJournal.TARGET_TAG_LIST,
            recursive=True,
        )
        print(f"[debug]type(splitted_soup):{type(splitted_soup)}")

        for element in splitted_soup:
            if element.name == GummyAbstJournal.SECTION_TAG:
                tag_text_original = str_strip(element.get_text())
            if element.name == GummyAbstJournal.IMAGE_TAG:
                tag_text_original = src2base64(base=self.crawling_logs.get("cano_url"), src=element)
            elif element.name == GummyAbstJournal.SUBHEADER_TAG:
                tag_text_original = str_strip(element.get_text())
            else:
                tag_text_original = self.arrange_english(element.get_text())
            contents_in_one_section.append(ContentInSection(element, element.name, tag_text_original))

        return contents_in_one_section

    # TODO: これ意味ある?
    @staticmethod
    def arrange_english(english: str):
    ...
    @staticmethod
    def get_tex_url(url: str):
        """Convert the URL to the URL of the tex page you access when ``crawl_type=="tex"``"""
        return url

    def get_contents_tex(self, url: str, driver: Optional[webdriver.Chrome] = None) -> Tuple[str, List[Dict]]:
    ...
    def get_tex_source(self, url: str, driver: Optional[webdriver.Chrome] = None) -> str:
    ...
    # TODO: 現時点では無意味なメソッド
    def get_title_from_tex(self, tex: str):
    ...
    def get_contents_from_tex_sections(self, tex_sections: List[str]) -> List[Dict]:
    ...
    def get_contents_pdf(self, url: str, driver: Optional[webdriver.Chrome] = None):
    ...
    def get_pdf_source(self, url, driver: Optional[webdriver.Chrome] = None):
...
class PDFCrawler(GummyAbstJournal):
    ...
    def get_contents_pdf(self, url, driver: Optional[webdriver.Chrome] = None):
...
class NCBICrawler(GummyAbstJournal):
    ...
    def get_sections_from_soup(self, soup):
        sections = [
            ...
            if not find_target_text(soup=e, target_tag="h2", default_value="OK").lower().startswith("reference")
        ]
        return sections

    def get_header_tag_from_section(self, section):
...
class OxfordAcademicCrawler(GummyAbstJournal):
    ...
    def get_sections_from_soup(self, soup):
        sections = []
        article_full_text = soup.find(
            name="div",
            class_="widget-items",
            attrs={"data-widgetname": "ArticleFulltext"},
        )
...
class ScienceDirectCrawler(GummyAbstJournal):
    ...
    def get_title_from_soup(self, soup):
        title = find_target_text(
            ...
            class_="title-text",
            strip=True,
            default_value=self.default_title,
        )
        return title
...
class SpringerCrawler(GummyAbstJournal):
    ...
    def get_title_from_soup(self, soup):
        title = find_target_text(
            soup=soup,
            target_tag=GummyAbstJournal.TITLE_TAG,
...
class UniOKLAHOMACrawler(GummyAbstJournal):
    ...
    def get_contents_from_soup_sections(self, soup_sections):
        ...
        len_soup_sections = len(soup_sections)
        for i, section in enumerate(soup_sections):
            contents.extend(self.organize_soup_section(section_tag=section, header="", head_is_not_added=False))
            if self.verbose:
...
class LungCancerCrawler(GummyAbstJournal):
    ...
    def get_sections_from_soup(self, soup):
        sections = [
            ...
            if find_target_text(soup=e, target_tag=("h2", "h3"), default_value="") not in self.AvoidHead
        ]
        return sections
...
class WileyOnlineLibraryCrawler(GummyAbstJournal):
    ...
    def get_sections_from_soup(self, soup):
        sections = soup.find_all(
            name="section",
            class_=("article-section__abstract", "article-section__content"),
        )
        return sections
...
class UniKeioCrawler(GummyAbstJournal):
    ...
    def get_title_from_soup(self, soup):
        title = find_target_text(
            soup=soup, target_tag=GummyAbstJournal.TITLE_TAG, strip=True, default_value=self.default_title
        )
        return title
    ...
    def get_contents_from_soup_sections(self, soup_sections):
        ...
        for i, section in enumerate(soup_sections):
            ...
            contents.extend(self.organize_soup_section(section_tag=section, header=head))
            if self.verbose:
...
class frontiersCrawler(GummyAbstJournal):
    ...
    def get_title_from_soup(self, soup):
        ...
        if abst is None:
            abst = soup
        title = find_target_text(
            soup=abst, target_tag=GummyAbstJournal.TITLE_TAG, strip=True, default_value=self.default_title
        )
        return title
...
class TaylorandFrancisOnlineCrawler(GummyAbstJournal):
    ...
    def get_sections_from_soup(self, soup):
        sections = soup.find_all(
            name="div",
            class_=("hlFld-Abstract", "NLM_sec NLM_sec-type_intro NLM_sec_level_1"),
        )
        return sections
...
class JSSECrawler(GummyAbstJournal):
    ...
    def get_jsseNo(url):
        return re.sub(
            pattern=r"^.*\/index.php\/jsse\/article\/(?:view|download)\/([0-9]+)(?:\/[0-9]+)?$",
            repl=r"\1",
            string=url,
        )
...
class AMSCrawler(GummyAbstJournal):
    ...
    def get_title_from_soup(self, soup):
        title = find_target_text(
            soup=soup,
            target_tag=GummyAbstJournal.TITLE_TAG,
            class_="wi-article-title article-title-main",
            strip=True,
            default_value=self.default_title,
        )
        return title
    ...
    def get_sections_from_soup(self, soup):
        ...
        article = soup.find(
            name="div",
            attrs={"class": "widget-items", "data-widgetname": "ArticleFulltext"},
        )
        if article is not None:
...
class APSCrawler(GummyAbstJournal):
    ...
    def make_elements_visible(self, driver: webdriver.Chrome):
        ...
        while True:
            hidden_elements = driver.find_elements(
                by="xpath",
                value="//i[@class='fi-plus'][contains(@style,'display: block;')]",
            )
    ...
    def get_title_from_soup(self, soup):
        title = find_target_text(soup=soup, target_tag="h3", strip=True, default_value=self.default_title)
        return title
    ...
    def get_sections_from_soup(self, soup):
        ...
        if article is not None:
            for section in article.find_all(name="section"):
                if find_target_text(soup=section, target_tag="h2", default_value="OK").lower().startswith("reference"):
                    break
                sections.append(section)
        return sections
        ...
        if article is not None:
            sections.extend(
                group_soup_with_head(
                    soup=article,
                    name="h2",
                    class_="article-section__title section__title",
                )
            )
        return sections
...
class ARVOJournalsCrawler(GummyAbstJournal):
    ...
    def get_sections_from_soup(self, soup):
        ...
        if section is not None:
            section = replace_soup_tag(
                soup=section,
                new_name="strong",
                old_name="div",
                old_attrs={"class": "h7"},
            )
            for sec in split_section(section_tag=section, split_tag_names="div", class_="h6"):
    ...
    def get_sections_from_soup(self, soup):
        ...
        if section is not None:
            for sec in group_soup_with_head(soup=section, name="h2"):
                if find_target_text(soup=sec, target_tag="h2").lower().startswith("reference"):
                    break
                sections.append(sec)
        return sections
    ...
    def get_sections_from_soup(self, soup):
        ...
        if section is not None:
            ...
            for sec in article_sections:
                if find_target_text(soup=sec, target_tag="h2", default_value="head").lower().startswith("reference"):
                    break
                sections.append(sec)
        return sections
...
class BiomedGridCrawler(GummyAbstJournal):
    ...
    def get_title_from_soup(self, soup):
        title = find_target_text(
            soup=soup,
            target_tag=GummyAbstJournal.TITLE_TAG,
            class_="farticle-title citation_title",
            strip=True,
            default_value=self.default_title,
        )
        return title
    ...
    def get_sections_from_soup(self, soup):
        ...
        for sec in soup.find_all(name="section", class_="lighten-4"):
            head = self.get_header_tag_from_section(sec)
...
class NRRCrawler(GummyAbstJournal):
    ...
    def get_sections_from_soup(self, soup):
        ...
        for sec in group_soup_with_head(article, name="table"):
            if find_target_text(soup=sec, target_tag="table").lower().startswith("abstract"):
                start = True
            if find_target_text(soup=sec, target_tag="table").lower().startswith("reference"):
                break
            if start:
                sections.append(sec)
        return sections
    ...
    def get_sections_from_soup(self, soup):
        ...
        if article is not None:
            for sec in group_soup_with_head(article, name="div", class_="sectionHeading"):
                if (
                    find_target_text(soup=sec, target_tag="div", class_="sectionHeading")
                    .lower()
...
class PsycNetCrawler(GummyAbstJournal):
    ...
    @staticmethod
    def is_request_successful(soup: BeautifulSoup) -> None:
    ...
    def get_title_from_soup(self, soup):
        ...
        if article_title is not None:
            title = find_target_text(
                soup=article_title, target_tag=GummyAbstJournal.TITLE_TAG, strip=True, default_value=self.default_title
            )
        return title
...
class ChemRxivCrawler(GummyAbstJournal):
    ...
    def get_pdf_url(url):
        soup = BeautifulSoup(
            markup=requests.get(ChemRxivCrawler.get_soup_url(url)).content,
