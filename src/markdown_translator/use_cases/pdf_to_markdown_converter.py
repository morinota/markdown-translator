...
from markdown_translator.use_cases.pdf_2_md_tag_classfier import PDF2MdTagClassifier
...
class PDF2MarkdownConverter:
    ...
    def convert(
        ...
        for pdf_content in pdf_contents:
            ...
            if (
                md_tag == MarkdownTagName.PLAIN_TEXT
                and previous_md_type == MarkdownTagName.PLAIN_TEXT
            ):
                previous_plain_text_md = md_contents.pop()
                raw_text += previous_plain_text_md.text_raw + " "

            raw_text += pdf_content.raw_text
            raw_text_processed = re.sub(
                r"\n", "", raw_text
            )  # pdfの全てのtextlineに含まれている改行文字を削除
