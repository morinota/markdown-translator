...
def to_html(
    ...
    output_path = sanitize_filename(fp=output_path, ext=".html")
    ...
    if verbose:
        print(f"Save HTML file at {toBLUE(output_path)}")
    return output_path
...
def toPDF(path, title="", contents=[], searchpath=TEMPLATES_DIR, template="paper.html", verbose=True, options={}):
    ...
    html_path = to_html(
        output_path=html_path, title=title, contents=contents, searchpath=searchpath, template=template, verbose=verbose
    )
