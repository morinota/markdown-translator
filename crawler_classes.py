...
class NCBICrawler(GummyAbstJournal):
    ...
    def get_title_from_soup(self, soup):
        title = find_target_text(
            soup=soup, target_tag="title", class_="content-title", default_value=self.default_title
        )
        return title
...
class PubMedCrawler(GummyAbstJournal):
    ...
    def get_contents_soup(
        self, url, driver: Optional[webdriver.Chrome] = None, **gatewaykwargs
    ) -> Tuple[str, List[Dict]]:
        # html
...
class MDPICrawler(GummyAbstJournal):
    ...
    def get_soup_source(self, url: Optional[str] = None, driver: Optional[webdriver.Chrome] = None, **gatewaykwargs):
    ...
    def get_title_from_soup(self, soup):
        title = find_target_text(
            soup=soup, target_tag="title", class_="title", strip=True, default_value=self.default_title
        )
        return title
...
class JSTAGECrawler(GummyAbstJournal):
    ...
    def get_soup_source(self, url, driver: Optional[webdriver.Chrome] = None, **gatewaykwargs):
        cano_url = canonicalize(url=url, driver=driver)
        if self.verbose:
...
class UniKeioCrawler(GummyAbstJournal):
    ...
    def get_contents_soup(self, url: str, driver: Optional[webdriver.Chrome] = None, **gatewaykwargs):
        soup = self.get_soup_source(url=url, driver=driver, **gatewaykwargs)
    ...
    def get_title_from_soup(self, soup):
        title = find_target_text(soup=soup, target_tag="h1", strip=True, default_value=self.default_title)
        return title
...
class frontiersCrawler(GummyAbstJournal):
    ...
    def get_title_from_soup(self, soup):
        ...
        if abst is None:
            abst = soup
        title = find_target_text(soup=abst, target_tag="h1", strip=True, default_value=self.default_title)
        return title
...
class RNAjournalCrawler(GummyAbstJournal):
    ...
    def get_soup_source(self, url: str, driver: Optional[webdriver.Chrome] = None, **gatewaykwargs):
        cano_url = canonicalize(url=url, driver=driver)
        if self.verbose:
...
class IntechOpenCrawler(GummyAbstJournal):
    ...
    def get_title_from_soup(self, soup):
        title = find_target_text(
            soup=soup, target_tag="h1", class_="title", strip=True, default_value=self.default_title
        )
        return title
...
class PsycNetCrawler(GummyAbstJournal):
    ...
    def get_title_from_soup(self, soup):
        ...
        if article_title is not None:
            title = find_target_text(soup=article_title, target_tag="h1", strip=True, default_value=self.default_title)
        return title
...
class MinervaMedicaCrawler(GummyAbstJournal):
    ...
    def get_title_from_soup(self, soup):
        title = find_target_text(soup=soup, target_tag="title", strip=True, default_value=self.default_title)
        return title
...
class ChemRxivCrawler(GummyAbstJournal):
    ...
    def get_title_from_soup(self, soup):
        title = find_target_text(
            soup=soup, target_tag="h1", strip=True, default_value=self.default_title
        )  # class_="_3lGK4"
        return title
